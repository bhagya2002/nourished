...
const Notifications = () => {
  ...
  const handleDeclineInvite = async (inviteId: string) => {
    ...
    try {
      const declineInviteResponse = await fetch(`${API_BASE_URL}/declineInvite`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!declineInviteResponse.ok) throw new Error('Failed to decline invite');
      // Success, remove the invite from the list
      setNotifications(prevInvites => prevInvites.filter(invite => invite.id !== inviteId));
    } catch (error) {
      console.error(error);
  ...
  return (
    <Box>
      ...
      <Menu
        ...
        {/* For Friend Requests */}
        <ListSubheader sx={{ fontWeight: 'bold', userSelect: 'none' }}>
          {notifications.filter(notification => notification.type === 0).length > 0 && 'Friend Requests'}
        </ListSubheader>
        ...
        <ListSubheader sx={{ fontWeight: 'bold', userSelect: 'none' }}>
          {notifications.filter(notification => notification.type === 1).length > 0 && 'Challenges Invites'}
        </ListSubheader>
