...
const UserSearchDialog: React.FC<UserSearchDialogProps> = ({
  ...
  const fetchRecommendedUsers = async () => {
    ...
    try {
      ...
      if (recommendedUsersData && recommendedUsersData.data && Array.isArray(recommendedUsersData.data)) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (searchResultsData && searchResultsData.data && Array.isArray(searchResultsData.data)) {
  ...
  const handleFollow = async (type: number, followee: string) => {
    try {
      ...
      setSearchResults(searchResults.map((theUser) => {
        if (theUser.id !== followee) return theUser;
        if (theUser.followers?.includes(user.uid)) {
          theUser.followers = theUser.followers.filter((followerId) => followerId !== user.uid);
          return theUser;
        }
      ...
      setRecommendedUsers(recommendedUsers.map((theUser) => {
        if (theUser.id !== followee) return theUser;
        if (theUser.followers?.includes(user.uid)) {
          theUser.followers = theUser.followers.filter((followerId) => followerId !== user.uid);
          return theUser;
      ...
      <DialogContent sx={{ pb: 1 }}>
        <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
          <TextField
            ...
            helperText={isSearchInputError ? "Please enter name / email to search" : ""}
            sx={{ my: 1 }}
            ...
            autoFocus
            value={keyword}
            onChange={(e) => { setKeyword(e.target.value); setIsSearchInputError(false); }}
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
                handleSearch();
              }
            ...
            autoCorrect="off"
          />
          <Button sx={{ my: 1, height: 52 }} variant="contained" startIcon={<KeyboardReturn />} onClick={handleSearch}>
            Search
          </Button>
        </Box>
        {user && (
          <Card>
            ...
            <Box sx={{ py: 1, px: 2 }}>
              {searchResults.length === 0 && (
                <Box sx={{ py: 1, px: 0, fontWeight: 'bold', letterSpacing: '0.5px' }}>
                  NOTHING...
                </Box>
              )}
              {searchResults.map((theUser: SearchedUser) => (
                <Box key={theUser.id} sx={{ display: 'flex', alignItems: 'center', py: 1 }}>
                  <Box sx={{ mr: 1 }}>
                    <AccountCircle />
                  </Box>
