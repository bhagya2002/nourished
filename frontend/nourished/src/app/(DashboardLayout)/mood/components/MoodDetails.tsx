...
const MoodDetails: React.FC<MoodDetailsProps> = ({
  ...
  const handleUpdate = async () => {
    ...
    setIsProcessing(true);
    try {
      await onUpdate(entry.date, note, selectedRating !== null ? selectedRating : entry.rating);
      setIsEditing(false);
    } catch (error) {
    ...
    >
      ...
      <DialogContent sx={{ pt: 3, pb: 1 }}>
          ...
          >
            ...
            <Box sx={{ textAlign: 'center', mb: 1 }}>
              <Typography
                variant="body2"
                sx={{
                  color: selectedRating !== null ? getMoodInfo(selectedRating).color : 'text.secondary',
                  fontWeight: 500,
                }}
              >
                {selectedRating !== null ? getMoodInfo(selectedRating).label : 'Select a mood'}
              </Typography>
            </Box>
          </Box>
        )}
        ...
        {isDeleting && (
          ...
          >
            <Typography variant="body1" sx={{ mb: 1, color: theme.palette.error.main }}>
      ...
      <DialogActions
        sx={{
          ...
          pt: 2,
          display: 'flex',
          justifyContent: isEditing || isDeleting ? 'space-between' : 'flex-end'
        }}
      >
        ...
        {isEditing && !isDeleting && (
          <>
            ...
            <Button
              ...
              variant="contained"
              disabled={isProcessing || (note === entry.note && selectedRating === entry.rating)}
            >
