...
module.exports.acceptInvite = async function acceptInvite(uid, data) {
    try {
        ...
        switch (data.type) {
            case 0: { //Friend invite
                batch.update(db.getRef("users", data.invitee), { friends: db.getAddToArray(uid) });
                batch.update(db.getRef("users", uid), { friends: db.getAddToArray(data.invitee) });
                const result = await db.commitBatch(batch);
                return result;
            } case 1: { // Challenge invite
                batch.update(db.getRef("users", data.invitee), { challenges: db.getAddToArray(data.targetId) });
                batch.update(db.getRef("challenges", data.targetId), { participants: db.getAddToArray(data.invitee) });
                const result = await db.commitBatch(batch);
                return result;
            }
        }
