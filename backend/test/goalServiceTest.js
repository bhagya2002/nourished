...
describe("goalService", function () {
    ...
    describe("createGoal", function () {
        it("should create a goal and return its ID", async function () {
            ...
            expect(result).to.deep.equal({ success: true, data: { id: "goal123", challengeId: null } });
        });
        ...
        it("should create a goal and a challenge if isChallenge is true", async function () {
            ...
            dbStub.addSingleDoc.resolves({ success: true, id: "goal123" });
            dbStub.updateFieldArray.resolves({ success: true });
            challengeServiceStub.createChallenge.resolves({ success: true, data: { id: "challenge123" } });

            const result = await goalService.createGoal(uid, goal, ["user456"]);
            expect(result).to.deep.equal({ success: true, data: { id: "goal123", challengeId: "challenge123" } });
        });

        it("should return an error if goal creation fails", async function () {
            const uid = "user123";
            const goal = { title: "New Goal" };
            dbStub.addSingleDoc.resolves({ success: false, error: "Failed to create goal" });

            const result = await goalService.createGoal(uid, goal, []);
            expect(result).to.deep.equal({ success: false, error: "Failed to create goal" });
        });
        ...
        it("should return an error if updating user goals fails", async function () {
            ...
            const goal = { title: "New Goal" };
            dbStub.addSingleDoc.resolves({ success: true, id: "goal123" });
            dbStub.updateFieldArray.resolves({ success: false, error: "Failed to update user goals" });

            const result = await goalService.createGoal(uid, goal, []);
            expect(result).to.deep.equal({ success: false, error: "Failed to update user goals" });
        });
        ...
        it("should return an error if challenge creation fails", async function () {
            ...
            dbStub.addSingleDoc.resolves({ success: true, id: "goal123" });
            dbStub.updateFieldArray.resolves({ success: true });
            challengeServiceStub.createChallenge.resolves({ success: false, error: "Failed to create challenge" });
    ...
    describe("editGoal", function () {
        it("should edit a goal successfully", async function () {
            ...
            const result = await goalService.editGoal(uid, goalId, fieldToChange, newValue);
    ...
    describe("getUserGoals", function () {
        it("should return user goals successfully", async function () {
            ...
            dbStub.queryDatabaseSingle.resolves({ success: true, data: { goals: ["goal1", "goal2"] } });
    ...
    describe("deleteGoal", function () {
        it("should delete a goal and its challenges successfully", async function () {
            ...
            dbStub.queryDatabaseSingle.resolves({ success: true, data: { uid: "user123" } });
            dbStub.queryDatabase.resolves({ success: true, data: [{ id: "challenge1" }] });
