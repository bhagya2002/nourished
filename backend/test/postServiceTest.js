...
describe("postService", function () {
    ...
    describe("createPost", function () {
        it("should create a post successfully", async function () {
            ...
            dbStub.queryDatabaseSingle.resolves({ success: true, data: { title: "Goal Title" } });
        ...
        it("should return an error if post creation fails", async function () {
            ...
            dbStub.addSingleDoc.resolves({ success: false, error: "Failed to create post" });

            const result = await postService.createPost(uid, post);
            expect(result).to.deep.equal({ success: false, error: "Failed to create post" });
        });
        ...
        it("should return an error if updating user posts fails", async function () {
            ...
            const post = { name: "Post Name" };
            dbStub.addSingleDoc.resolves({ success: true, id: "post123" });
            dbStub.updateFieldArray.resolves({ success: false, error: "Failed to update user posts" });

            const result = await postService.createPost(uid, post);
            expect(result).to.deep.equal({ success: false, error: "Failed to update user posts" });
        });
        ...
        it("should return an error if goal retrieval fails", async function () {
            ...
            dbStub.addSingleDoc.resolves({ success: true, id: "post123" });
            dbStub.updateFieldArray.resolves({ success: true });
            dbStub.queryDatabaseSingle.resolves({ success: false, error: "Goal not found" });
    ...
    describe("editPost", function () {
        it("should edit a post successfully", async function () {
            ...
            const result = await postService.editPost(uid, postId, fieldToChange, newValue);
            expect(result.success).to.be.true;
        });
    ...
    describe("getUserWithFriendPosts", function () {
        it("should return posts from user and friends", async function () {
            ...
            sinon.stub(postService, "getUserPosts")
                .withArgs("user123").resolves({ success: true, data: [{ id: "post1" }] })
                .withArgs("friend1").resolves({ success: true, data: [{ id: "post2" }] })
                .withArgs("friend2").resolves({ success: true, data: [{ id: "post3" }] });
    ...
    describe("getUserPosts", function () {
        it("should return user posts with goals", async function () {
            ...
            dbStub.queryDatabaseSingle.resolves({ success: true, data: { posts: ["post1"] } });
    ...
    describe("deletePost", function () {
        ...
        it("should return an error if removing post from user fails", async function () {
            const uid = "user123";
            const postId = "post123";
            dbStub.removeFromFieldArray.resolves({ success: false, error: "Failed to remove post" });

            const result = await postService.deletePost(uid, postId);
            expect(result).to.deep.equal({ success: false, error: "Failed to remove post" });
        });
    });
    ...
    describe("likePost", function () {
        ...
        it("should return an error if post is not found", async function () {
            const uid = "user123";
            const postId = "post123";
            dbStub.queryDatabaseSingle.resolves({ success: false, error: "Post not found" });
